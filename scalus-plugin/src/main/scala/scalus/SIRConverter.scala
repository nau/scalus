...
class SIRConverter(using Context) {
  ...
  // FIXME generate a method for each let binding
  // to avoid a Java limit on the size of a method
  // Generated bytecode for method 'blah' is too large. Limit is 64KB
  // which arises when scripts are too large

  /* def mkLetDef(recursivity: Recursivity, bindings: List[Binding], body: SIR) =
    val let = mkLet(recursivity, bindings, body)
    val err = mkError("TestError")
