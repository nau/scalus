...
final class SIRCompiler(mode: scalus.Mode)(using ctx: Context) {
        ...
        val dataDecl = globalDataDecls.get(FullName(dataTypeSymbol)) match
            ...
            case None =>
                ...
                val constrDecls = sortedConstructors.map { sym =>
                    val params = primaryConstructorParams(sym).map(p => TypeBinding(p.name.show, SIRTypesHelper.sirType(p.info, srcPos)))
                    ...
                    val baseTypeArgs = optBaseClass.flatMap{ bs =>
                        sym.info.baseType(bs) match
                            case AppliedType(_, args) => Some(args.map(a => SIRTypesHelper.sirType(a, srcPos)))
                            case _ => None
                    }.getOrElse(Nil)
    ...
    private def compileIdentOrQualifiedSelect(env: Env, e: Tree): SIRExpr = {
        ...
        (isInLocalEnv, isInGlobalEnv) match
            ...
            case (false, true) =>
                SIR.Var(e.symbol.fullName.toString(), SIRTypesHelper.sirType(e.tpe.widen, e.srcPos))
            case (false, false) =>
                mode match
                    case scalus.Mode.Compile =>
                        ...
                        SIR.ExternalVar(
                          ...
                          e.symbol.fullName.toString(),
                          SIRTypesHelper.sirType(e.tpe.widen, e.srcPos)
                        )
    ...
    private def compileValDef(env: Env, vd: ValDef): CompileMemberDefResult = {
        ...
        // ignore @Ignore annotated statements
            ...
            CompileMemberDefResult.Ignored(SIRTypesHelper.sirType(vd.tpe, vd.srcPos))
        ...
        then
            ...
            CompileMemberDefResult.Builtin(name.show, SIRTypesHelper.sirType(vd.tpe, vd.srcPos))
        else
    ...
    private def compileDefDef(env: Env, dd: DefDef, isGlobalDef: Boolean): CompileMemberDefResult = {
        ...
        if dd.symbol.flags.is(Flags.Inline) || dd.symbol.hasAnnotation(IgnoreAnnot) then
            CompileMemberDefResult.Ignored(SIRTypesHelper.sirType(dd.tpe, dd.srcPos))
            // ignore PlatformSpecific statements
            // NOTE: check ps.tpe is not Nothing, as Nothing is a subtype of everything
        else
            // TODO store comments in the SIR
            ...
            val paramNameTypes =
                ...
                else params.map { case v: ValDef => (v.symbol.name.show, SIRTypesHelper.sirType(v.tpe, v.srcPos)) }
            val body = dd.rhs
            val selfName = if isGlobalDef then FullName(dd.symbol).name else dd.symbol.name.show
            val bE = compileExpr(env ++ paramNameTypes + (selfName -> SIRTypesHelper.sirType(dd.tpe, dd.srcPos)) , body)
    ...
    private def compileStmt(env: Env, stmt: Tree, isGlobalDef: Boolean = false): CompileMemberDefResult = {
    ...
    private def compileBlock(env: Env, stmts: immutable.List[Tree], expr: Tree): SIRExpr = {
    ...
    private def compileBigIntOps(env: Env, lhs: Tree, op: Name, rhs: Tree, optree: Tree): SIRExpr =
        op match
            case nme.PLUS =>
                SIR.Apply(
    ...
    private def compileMatch(tree: Match, env: Env): SIRExpr = {
        pmCompiler.compileMatch(tree, env)
    }

    private def compileBuiltinPairMethods(env: Env, fun: Name, pair: Tree, tree: Tree): SIRExpr =
        fun.show match
            case "fst" =>
                ...
                expr.tp match
                    ...
                    case other =>
                        error(TypeMismatch(fun.toString,SIRType.Pair(SIRType.TypeVar("A"),SIRType.TypeVar("B")),other,tree.srcPos), SIR.Error(""))
            case "snd" =>
                val expr = compileExpr(env,pair)
                expr.tp match
                    case SIRType.Pair(t1, t2) =>
    ...
    private def compileBuiltinPairConstructor(
        ...
        if a.isLiteral && b.isLiteral then
            SIR.Const(
              ...
              SIRType.Pair(SIRTypesHelper.sirType(a.tpe, a.srcPos),SIRTypesHelper.sirType(b.tpe, b.srcPos))
            )
    ...
    private def compileBuiltinListMethods(env: Env, lst: Tree, fun: Name): SIRExpr =
        fun.show match
            case "head" =>
    ...
    private def compileBuiltinListConstructor(
        ...
        val tpeTp = SIRTypesHelper.sirType(tpe.tpe, tree.srcPos)
        ...
        ex match
            case SeqLiteral(args, _) =>
                ...
                if allLiterals then
                    ...
                    val nil: SIRExpr = SIR.Const(scalus.uplc.Constant.List(tpeE, Nil), SIRType.List.Nil )
                    args.foldRight(nil) { (arg, acc) =>
                        SIR.Apply(
                          SIR.Apply(SIRBuiltins.mkCons, compileExpr(env, arg), SIRType.Fun(listTp,listTp)),
                          acc,
                          listTp
                        )
                    }
    ...
    private def compileApply(env: Env, f: Tree, args: List[Tree], applyTpe: Type, applyTree: Apply): SIRExpr = {
        ...
        val sirType = SIRTypesHelper.sirType(applyTpe, applyTree.srcPos)
        val argsE = args.map(compileExpr(env, _))
        if argsE.isEmpty then SIR.Apply(fE, SIR.Const(scalus.uplc.Constant.Unit, SIRType.VoidPrimitive), sirType)
        else
            // (f : (arg1 -> args2 -> ... -> res))
            // Apply(f, arg1) arg2 -> ... -> res)
            //  ....
            // Apply(...Apply(... f, arg1), arg2),,, ) res)
            val partTpes = argsE.foldRight(sirType)((a, acc) => SIRType.Fun(a.tp, acc))
            val (applyExpr, applyTpe) = argsE.foldLeft((fE,partTpes)){  (acc, arg) =>
                val (fun, tp) = acc
                val nTp = tp match
                    case SIRType.Fun(t1, t2) => t2
                    case _ =>
                        error(TypeMismatch("Function type",SIRType.Fun(SIRType.TypeVar("A"),SIRType.TypeVar("B")),tp,f.srcPos), SIRType.VoidPrimitive)
    ...
    def compileExpr[T](env: Env, tree: Tree)(using Context): SIRExpr = {
        ...
        if compileConstant.isDefinedAt(tree) then
            ...
            SIR.Const(const, SIRTypesHelper.sirType(tree.tpe, tree.srcPos))
    ...
    private def compileExpr2(env: Env, tree: Tree)(using Context): SIRExpr = {
        tree match
            case If(cond, t, f) =>
                ...
                val sirTp = SIRTypesHelper.sirType(tree.tpe, tree.srcPos)
           ...
           So we just ignore this PlatformSpecific argument and compile the rest
            ...
            case Apply(Select(lhs, op), List(rhs))
                ...
                if !(rhs.tpe.widen =:= converter.ByteStringClassSymbol.typeRef) then
                    ...
                    val rhsExpr = compileExpr(env, rhs)
                    val eq = SIR.Apply(
                      SIR.Apply(SIRBuiltins.equalsByteString, lhsExpr, SIRType.liftM[ByteString=>Boolean]),
                      rhsExpr,
                      SIRType.BooleanPrimitive
                    )
            ...
            case Apply(Select(lhs, op), List(rhs))
                ...
                val eq =
                    SIR.Apply(
                        SIR.Apply(SIRBuiltins.equalsString, lhsExpr, SIRType.liftM[String=>Boolean]),
                        rhsExpr,
                        SIRType.BooleanPrimitive)
                if op == nme.EQ then eq else SIR.Not(eq)
            // Data equality
            ...
            case tree @ TypeApply(Select(list, name), immutable.List(tpe))
                ...
                SIR.Const(scalus.uplc.Constant.List(tpeE, Nil),SIRType.List(SIRTypesHelper.sirType(tpe.tpe, tree.srcPos)))
            case Apply(
                  TypeApply(Select(list, name), immutable.List(tpe)),
                  immutable.List(arg)
                ) if name == termName("::") && list.isList =>
                val argE = compileExpr(env, arg)
                val exprType = SIRTypesHelper.sirType(tpe.tpe, tree.srcPos)
                SIR.Apply(
            ...
            sel.srcPos
                ...
                if ts.isClass && fieldIdx >= 0 then
                    ...
                    val selType = SIRTypesHelper.sirType(sel.tpe, sel)
                    val s0: SIRExpr = SIR.Var(ident.show, selType)
                    val lam = primaryConstructorParams(ts).foldRight(s0) {
                        case (f, acc) =>
                            SIR.LamAbs(SIR.Var(f.name.show, SIRTypesHelper.sirType(f.paramInfo, sel)), acc)
                    }
            ...
            case Block(
                ...
                compileStmt(env, dd) match
                    ...
                    case CompileMemberDefResult.Ignored(tp) => 
                        error(GenericError("Ignoring closure", tree.srcPos), SIR.Error("Ignored closure"))
                    case CompileMemberDefResult.Builtin(name, tp) => 
                        error(GenericError("Builtin library can be part of user code", tree.srcPos), SIR.Error("Builtin definition"))    
                    case _ => 
...
object SIRCompiler {
    ...
    case class Env(
        ...
        def ++(bindings: Iterable[(String, SIRType)]): Env = Env(vars ++ bindings, typeVars)

        def +(ntpe: (String, SIRType)): Env = Env(vars + ntpe, typeVars)
