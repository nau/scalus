...
class InlinerSpec extends AnyFunSuite {
    ...
    test("should handle Constr and Case") {
        ...
        val caseExpr = Case(
          constr,
          List(
            λ("x")(x => x) $ vr"y", // Identity function application
            0
          )
        )
    ...
    test("should handle multiple variable references") {
        ...
        val term = λ("x")(x => AddInteger $ x $ (MultiplyInteger $ x $ x)) $ 42
