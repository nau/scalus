    ...
    def liftMImpl[T <: AnyKind: Type](using Quotes): Expr[SIRType] = {
        ...
        def liftRepr(tp: TypeRepr, env: Env): SIRType = {
            tp match
                case typeLambda @ quotes.reflect.TypeLambda(paramNames, paramBounds, resType) =>
                    ...
                    val sirParamExprs = paramNames.zipWithIndex.map { case (name, idx) =>
                ...
                case tpc: TypeRef =>
                    ...
                    if (sym == defn.NothingClass) then SIRType.TypeNothing
                    else if (sym.isAliasType) then liftRepr(tpc.dealias, env)
                    else if (sym.isClassDef) then liftClassType(tpc, Nil, env)
                ...
                case other =>
                    tp.asType match {
                        ...
                        case '[scalus.builtin.Pair[a, b]] =>
                            ...
                            val a = TypeRepr.of[a]
                            val b = TypeRepr.of[b]
                            SIRType.Pair(liftRepr(a, env), liftRepr(b, env))
                        case '[(a, b)] =>
        ...
        def liftClassType(tp: TypeRepr, typeArgs: List[SIRType], env: Env): SIRType = {
            ...
            val retval = (tryLiftPrimitive(sym, typeArgs) orElse
                ...
                tryMakeNonCaseModule(tp, sym, typeArgs, env)).getOrElse {
                val name = sym.name
                val typeArgsShow = typeArgs.map(_.show)
                unsupportedType(
                  tp,
                  s"tree=${tp}, isClass=${sym.isClassDef} isAliasType=${sym.isAliasType}",
                  env
                )
            }
            retval
        ...
        def tryLiftPrimitive(sym: Symbol, types: List[SIRType]): Option[SIRType] = {
            if (sym == defn.IntClass || sym == defn.LongClass) then Some(IntegerPrimitive)
            else if (sym == Symbol.requiredClass("scalus.builtin.ByteString")) then
                Some(ByteStringPrimitive)
            else if (sym == Symbol.requiredClass("scala.math.BigInt")) then Some(IntegerPrimitive)
            else if (sym == defn.StringClass) then Some(StringPrimitive)
            else if (sym == defn.BooleanClass) then Some(BooleanPrimitive)
            else if (sym == defn.UnitClass) then Some(VoidPrimitive)
            else None
        }

        def tryLiftBuildinType(sym: Symbol, types: List[SIRType], env: Env): Option[SIRType] = {
            if (sym == Symbol.requiredClass("scalus.builtin.Data")) then Some(Data)
        ...
        def tryLiftCaseClassOrCaseParent(
            tp: TypeRepr,
            sym: Symbol,
            targs: List[SIRType],
            env: Env
        ...
        def tryLiftCaseClassOrCaseParentNoProxy(
            tp: TypeRepr,
            symbol: Symbol,
            targs: List[SIRType],
            env: Env
        ...
        def unsupportedType(
            ...
            if (throwException) then throw new IllegalArgumentException(fullMessage)
