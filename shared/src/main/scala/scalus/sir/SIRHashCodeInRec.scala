...
object SIRHashCodeInRec  {
   ...
   given SIRHashCodeInRec[SIRType] with {
       ...
       def applyEq(a: SIRType, y: SIRType, trace: java.util.IdentityHashMap[SIRType,List[SIRType]]): Boolean = {
         (a,y) match
   ...
   given SIRHashCodeInRec[SIRType.Fun] with {
       ...
       def applyEq(a: SIRType.Fun, b: SIRType.Fun, trace: java.util.IdentityHashMap[SIRType,List[SIRType]]): Boolean = {
   ...
   given SIRHashCodeInRec[SIRType.SumCaseClass] with {
       ...
       def applyEq(a: SIRType.SumCaseClass, b: SIRType.SumCaseClass, trace: java.util.IdentityHashMap[SIRType,List[SIRType]]): Boolean = {
           val inTrace = trace.get(a)
           if (inTrace eq null) || !inTrace.contains(b) then
    ...
    given SIRHashCodeInRec[SIRType.CaseClass] with {
         ...
         def apply(cc: SIRType.CaseClass, trace: java.util.IdentityHashMap[SIRType, Int]): Int = {
            ...
            def applyEq(a: SIRType.CaseClass, b: SIRType.CaseClass, trace: java.util.IdentityHashMap[SIRType,List[SIRType]]): Boolean = {
                val inTrace = trace.get(a)
                if (inTrace eq null) || !inTrace.contains(b) then
                    val nSameAsA = if (inTrace eq null) then List(b) else b :: inTrace
                    trace.put(a,nSameAsA)
    ...
    given SIRHashCodeInRec[TypeBinding] with {
        ...
        override def applyEq(a: TypeBinding, b: TypeBinding, trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
            a.name == b.name &&
    ...
    given SIRHashCodeInRec[SIRType.TypeLambda] with {
        ...
        override def apply(t: SIRType.TypeLambda, trace: util.IdentityHashMap[SIRType, Int]): Int = {
            import scala.util.hashing.MurmurHash3
        ...
        override def applyEq(a: SIRType.TypeLambda, b: SIRType.TypeLambda, trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
            val inTrace = trace.get(a)
            if (inTrace eq null) || !inTrace.contains(b) then
                val nSameAsA = if (inTrace eq null) then List(b) else b :: inTrace
                trace.put(a, nSameAsA)
    ...
    given SIRHashCodeInRec[SIRType.TypeProxy] with {
        ...
        override def applyEq(a: SIRType.TypeProxy, b: SIRType.TypeProxy, trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
    ...
    given SIRHashCodeInRec[DataDecl] with {
        ...
        override def apply(t: DataDecl, trace: util.IdentityHashMap[SIRType, Int]): Int = {
            ...
            h = MurmurHash3.mix(h, summon[SIRHashCodeInRec[List[ConstrDecl]]](t.constructors,trace))
            h = MurmurHash3.finalizeHash(h,2)
            h
        }

        override def applyEq(a: DataDecl, b: DataDecl, trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
            a.name == b.name &&
            summon[SIRHashCodeInRec[List[ConstrDecl]]].applyEq(a.constructors, b.constructors, trace)
        }
    ...
    given SIRHashCodeInRec[ConstrDecl] with {
        ...
        override def applyEq(a: ConstrDecl, b: ConstrDecl, trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
            a.name == b.name &&
    ...
    given list[T:SIRHashCodeInRec]: SIRHashCodeInRec[List[T]] with {
        ...
        override def applyEq(a: List[T], b: List[T], trace: util.IdentityHashMap[SIRType, List[SIRType]]): Boolean = {
