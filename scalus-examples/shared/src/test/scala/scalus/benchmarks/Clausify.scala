...
class Clausify extends AnyFunSuite, ScalusTest:
    ...
    test("F1") {
        ...
        val result = Compiler.compile { Formula.Sym(BigInt(0)).clauses }.scriptV3().evaluateDebug
        println("--------------- " + result)
...
object Clausify:
    ...
    extension (self: Formula)
        ...
        def disin: Formula =
            self match
                case Or(left, right) =>
                    val isLeftAnd = left.isInstanceOf[And]
                    val isRightAnd = right.isInstanceOf[And]
                    if isRightAnd then
                        val And(rightArg1, rightArg2) = right.asInstanceOf[And]
                        (left || rightArg1).disin && (left || rightArg2).disin
                    else if isLeftAnd then
                        val And(leftArg1, leftArg2) = left.asInstanceOf[And]
                        (leftArg1 || right).disin && (leftArg2 || right).disin
                    else
                        val leftDisin = left.disin
                        val rightDisin = right.disin
                        if leftDisin.isInstanceOf[And] || rightDisin.isInstanceOf[And] then
                            (leftDisin || rightDisin).disin
